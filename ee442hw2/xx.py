...
void printThreadsStatus()
{
    ...
    for(int i = 1 ; i < 6; i++)
    {
        ...
        {
        case READY: readyStr.pos += sprintf(&readyStr.string[readyStr.pos], "T%d,", i); break;
        case RUNNING: runningStr.pos += sprintf(&runningStr.string[runningStr.pos], "T%d,", i);break;
        case FINISHED: finishedStr.pos += sprintf(&finishedStr.string[finishedStr.pos], "T%d,", i);break;
        case IO: ioStr.pos += sprintf(&ioStr.string[ioStr.pos], "T%d,", i);break;
        }
    }
    //remove commas end of string
    if(readyStr.string[readyStr.pos - 1] ==',' ) readyStr.string[readyStr.pos - 1] = '\0';
    if(runningStr.string[runningStr.pos - 1] ==',' ) runningStr.string[runningStr.pos - 1] = '\0';
    if(finishedStr.string[finishedStr.pos - 1] ==',' ) finishedStr.string[finishedStr.pos - 1] = '\0';
    if(ioStr.string[ioStr.pos - 1] ==',' ) ioStr.string[ioStr.pos - 1] = '\0';
...
{            
    ...
    getcontext(&threadArray[nextThreadIndex].context);
    printf("after swap\n");
}
...
void thread_func(int n){
    ...
    while(i < 10){
        ...
        threadArray[threadIndex].cpuburst[bursttime]--;
        ...
        printf("%d %d\n",++i,n);
        printf("bofre getconct\n");
        getcontext(&threadArray[threadIndex].context);
        printf("i: %d n:%d\n", i,n);
        printf("after getconct\n");
    }
